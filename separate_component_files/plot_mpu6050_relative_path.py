...
def _plot(
    ...
    heading_values = _heading_series(data, path.heading_axis)
    spans = _stationary_spans(data.time_s, data.stationary)
    ...
    if path.step_indices:
        ax_linear.scatter(
            [data.time_s[i] for i in path.step_indices],
            [data.linear_mag_g[i] for i in path.step_indices],
            color="tab:blue",
            s=25,
    ...
    ax_heading.plot(data.time_s, heading_values, label=f"gyro_angle_{path.heading_axis}_deg")
    if path.step_indices:
        ax_heading.scatter(
            [data.time_s[i] for i in path.step_indices],
...
def main() -> int:
    ...
    heading_axis = _choose_heading_axis(data, args.heading_axis)
    threshold_g = (
